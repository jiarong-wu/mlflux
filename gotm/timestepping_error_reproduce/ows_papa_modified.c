...
int syear = 2012, smonth = 3, sday = 21;
...
int main()
{
  ...
  // system ("test -f sst.dat || ( "
	...
	//   "wget -q " GOTM_CASES "ows_papa/sst.dat && "  // sst not needed
		   ...
		   int sy, int sm, int sd, scalar s)
{
  ...
  fprintf (stderr, "Pass all the data before starting date! n=%d \n", n);
  double zd[n], od[n];
  int i = 0;
  while (fscanf (fp, "%lf %lf", &zd[i], &od[i]) == 2 && i < n) {
    i++;
    count++;
    fprintf (stderr, "Loading line %d, which is total line %d \n", i, count);
  }  
  if (i != n) {
  ...
  fprintf (stderr, "Finish reading profile, start assign values!\n");
  double zc = zb[];
  foreach_layer() {
    zc += h[]/2.;
    int i;
